...
class Block(nn.Module):
    ...
    #def forward(self, x, rel_pos_bias=None):
    #    if self.gamma_1 is None:
    #        x = x + self.drop_path(checkpoint(self.attn, self.norm1(x), rel_pos_bias))
    #        x = x + self.drop_path(checkpoint(self.mlp, self.norm2(x)))
    #    else:
    #        x = x + self.drop_path(self.gamma_1 * checkpoint(self.attn, self.norm1(x), rel_pos_bias))
    #        x = x + self.drop_path(self.gamma_2 * checkpoint(self.mlp, self.norm2(x)))
    #    return x
...
@register_model
def beit_huge_patch14_392(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=392,
                              patch_size=14,
                              embed_dim=1280,
                              depth=32,
                              num_heads=16,
...
if __name__ == "__main__":
    ...
    state_dict = torch.load('/home/yanglingfeng/.cache/torch/hub/checkpoints/imagenet_acc88.0_beiT_vit_L_224.pth')
    ...
    if True:
        assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias'} or set(
            msg.missing_keys) == {'head.weight', 'head.bias'}
    else:
